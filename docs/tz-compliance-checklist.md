...
- Виправлено рендеринг тексту коментарів у SPA (`RootListPageComponent`, `ThreadPageComponent`): дозволені XHTML-теги (`<a>`, `<code>`, `<i>`, `<strong>`) тепер відображаються як форматований контент замість сирого тексту.
- Додано збереження переносів рядків під час відображення тексту коментарів (перетворення `\n` у `<br />` на UI-рівні).
- Виправлено такий самий рендеринг у fallback-UI (`Comments.Api/wwwroot/app.js`) для консистентної поведінки.
- Оновлено цей чекліст: прибрано неактуальні примітки, залишено лише поточний стан і план.
