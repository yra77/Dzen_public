...
Останнє оновлення: 2026-03-17 (ітерація 47).

## Що перевірено в цій ітерації

- Для Angular SPA розширено обробку API-помилок: додано ознаку `canRetry` для transient-збоїв (network/timeout/5xx/429), щоб UI міг показувати retry-friendly стан.
- Оновлено сторінки `root-list` і `thread`: при transient-помилках створення/відповіді показується окремий retry-hint без втрати введених даних.
- У нові/змінені методи сервісу додано коментарі (JSDoc) відповідно до домовленості про коментування коду.
- Актуалізовано backlog: крок по edge-case UX для мережевих/timeout збоїв переведено у виконане, наступний фокус — e2e smoke.
