...
Останнє оновлення: 2026-03-20 (ітерація graphql-contract-gate).
...
| GraphQL (HotChocolate) | ✅ Виконується за планом | Працює endpoint `/graphql`; додано script- та CI-перевірку контракту кореневих query/mutation полів для SPA-операцій. | Розширити контрактну перевірку до негативних сценаріїв валідації. |
...
| Elasticsearch (офіційний .NET client) | ⚠️ Частково | Пошук працює через HTTP-адаптери; додано resilient fallback на repository search при збоях Elasticsearch. | Перейти на офіційний .NET client Elasticsearch + typed mapping/templates. |
...
4. **P1 — GraphQL quality:** розширити контрактні перевірки GraphQL-операцій (валідація помилок/некоректних payload) у CI.
...
- Додано `scripts/check-graphql-contract.sh`: скрипт підіймає API (або працює проти вже запущеного endpoint) і валідовує наявність обовʼязкових GraphQL query/mutation полів.
- `scripts/go-no-go-check.sh` розширено до 5 кроків: додано GraphQL schema contract check перед e2e.
- Workflow `.github/workflows/comments-web-e2e-smoke.yml` оновлено: додано крок `Validate GraphQL schema contract`.
- Checklist актуалізовано: прибрано застарілі формулювання, зафіксовано новий фактичний стан по GraphQL quality.
...
- **P1 GraphQL:** додати негативні контрактні кейси (валідаційні помилки, некоректні payload, error shape) у CI.
