...
- `src/**` (C#):
  - виконано перевірку структури класів/інтерфейсів та наявності XML-коментарів.
  - зафіксовано, що більшість публічних типів уже задокументовані, але потрібен системний прохід по методах і файлових заголовках (див. розділ 6).
...
3. Додано окремий контрольний блок по коментарях у коді (класи/методи/файли).

## 6) Коментарі в коді: стан і що ще треба зробити

### Вже зроблено

- Проведено перевірку C#-коду в `src/**` на предмет документування типів.
- За результатом перевірки: більшість публічних класів/інтерфейсів уже має XML-коментарі.

### Що ще потрібно зробити обов’язково

1. **P0 — XML-коментарі до всіх публічних методів**
   - Додати/уніфікувати `/// <summary>` для публічних методів (Application/Infrastructure/Api).
   - Для методів з параметрами та поверненням — додати `/// <param>` і `/// <returns>`.
2. **P0 — Файлові заголовки**
   - Додати короткий верхній коментар у файли без контексту призначення (особливо handlers/consumers/options).
