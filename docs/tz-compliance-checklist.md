...
Останнє оновлення: 2026-03-16 (ітерація 3).

## Підсумок

- **Повністю виконано:** 22 пункти.
- **Частково виконано:** 4 пункти.
...
3. Перейти на **MediatR + FluentValidation** у Application-шарі.
4. Розширити RabbitMQ-пайплайн до окремих задач `indexing`/`file-processing`.
5. Розширити load-test до вимог Middle+ і зафіксувати метрики.
6. Додати відео-демо у README.
...
4. 🟨 **Клієнтська валідація parity** — довести UX-паритет із сервером для всіх кейсів sanitizer/XHTML помилок.
5. 🟨 **RabbitMQ черги `indexing` і `file-processing`** — додати прикладну маршрутизацію/консьюмери окремо від загальної події `comment.created`.
