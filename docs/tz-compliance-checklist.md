...
- Продовжено frontend-декомпозицію за планом: створено standalone-компонент `CommentNodeCardComponent` для уніфікованого рендерингу вузла коментаря (header, текст, вкладення, дія «Відповісти»).
- Підключено `CommentNodeCardComponent` у `RootListPageComponent` для рекурсивного дерева root-коментарів без дублювання шаблонного коду картки.
- Підключено `CommentNodeCardComponent` у `ThreadPageComponent` для кореня гілки та вкладених відповідей, з єдиною поведінкою відображення вкладень і reply-дії.
- Актуалізовано цей чекліст: видалено застарілі примітки, залишено тільки поточний стан відповідності та пріоритетний план.
