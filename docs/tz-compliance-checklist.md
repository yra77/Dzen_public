...
Останнє оновлення: 2026-03-22 (оновлено lightbox UX: кнопка закриття над зображенням, ефекти сповільнено; прибрано неактуальні формулювання).
...
- **Перегляд файлів із візуальними ефектами** — імплементовано: image-вкладення відкривається в lightbox-діалозі, кнопка закриття винесена над зображенням (не перекриває контент), анімації зроблено плавнішими.
...
| Lightbox для вкладень | ✅ | Додано модальний preview image-вкладень: кнопка закриття рендериться у верхньому правому куті над зображенням; працюють backdrop-ефект, плавний zoom/fade, закриття по кліку/кнопці. | Додати e2e перевірку відкриття/закриття lightbox + ESC. |
...
- Оновлено lightbox для image-вкладень:
  - кнопка `×` винесена у верхній правий кут **над зображенням** (контент не перекривається),
  - сповільнено ефекти `zoom-in` і `backdrop-fade` для м'якшого UX,
  - збережено закриття через кнопку `×` та клік по затемненій області.
- Checklist очищено: видалено неактуальні формулювання, залишено лише актуальні status/result і наступні кроки.
...
2. **P1: e2e покриття UX** — search/list/thread, lightbox (включно з ESC і перевіркою позиції кнопки `×` над зображенням), preview, realtime reconnect + пагінаційний summary-блок.
