...
- Продовжено frontend-декомпозицію за планом: у `CommentModalLayoutComponent` уніфіковано modal API (`closeMode`, `closeRequested`, `test-id` для layout/backdrop/panel).
- На сторінках `RootListPageComponent` і `ThreadPageComponent` підключено новий API модалок для create/reply сценаріїв, щоб закріпити єдину поведінку backdrop/Escape закриття.
- Актуалізовано чекліст: видалено застарілі формулювання про відсутність уніфікованого modal API.
