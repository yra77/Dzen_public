...
Останнє оновлення: 2026-03-20 (ітерація frontend-decomposition: shared form-state facade для thread reply flow).
...
3. **P1 — Frontend decomposition:** завершити декомпозицію state-management для форм (виділити shared facade/store для create/reply flow), зберігаючи сторінки `RootListPageComponent` і `ThreadPageComponent` тонкими; `ThreadPageComponent` уже переведено на shared facade, треба завершити `RootListPageComponent`.
...
- Додано shared facade `comment-form-state.facade.ts`, який централізує submit/preview/captcha/modal стани форми.
- `ThreadPageComponent` переведено на використання `CommentFormStateFacade`: template читає стан через getter-и фасаду, а imperative set*State-методи компонента видалено.
- Checklist оновлено за фактичним поточним станом і очищено від неактуальних проміжних нотаток.
...
- **P1 Frontend decomposition (продовження):** завершити міграцію `RootListPageComponent` (create + reply модалки) на shared facade/store, аналогічно вже оновленому `ThreadPageComponent`.
